...
def main():
    ...
    bg_query.Google_Cloud_query(logger, variable_table_loc, project_id, os.path.join(data_root, "variable_table.fits"))
    ...
    for wd_file in wd_files:
        ...
        wd_prefix = os.path.basename(wd_file).replace("_LC.fits", "")
        wd_name = wd_prefix.replace(f"{data_root}/lightcurves/", "")
        ...
        for filt in filters:
            ...
            analysis_initialise = bg_analysis(logger=logger, time=time, flux=ratio_norm, filename=results_filename)
